...
//Number of universes
//static const int N_UNIVERSES = 100;
...
class TemplateFitter {
  public:
    TemplateFitter(TemplateGroup_CC (&ccTemplates)[nbins_Ev], TemplateGroup_nue (&nueTemplates)[nbins_Ev], const std::vector<int>& dropBins = {}, bool PointLikeSource = false);
    ~TemplateFitter() {}
    ...
    bool doFit(int nFunctionCalls, int nIterations, double Tolerance, const char* varName[4], double seed[4], double stepSize[4], double (&parBf)[4], double (&parError)[4], double& chi2, bool fixedUt42 = false);
...
inline std::vector<std::pair<double, double>> GetDM2SeedsAndSteps(int nPoints_dm2) {
    if (nPoints_dm2 == 4) {
        ...
        return {
            {0.1, 1.0}, {0.3, 1.0}, {0.8, 1.0}, {2.0, 1.0},
            {7.0, 2.0}, {18.0, 2.0}, {50.0, 5.0}, {100.0, 10.0}
        };
...
constexpr int Stage3MaxCalls = 2000;
