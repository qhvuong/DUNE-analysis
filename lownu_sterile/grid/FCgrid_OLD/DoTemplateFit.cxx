...
//       double localPar[4], localErr[4], chi2_penalized_local;
...
//       tf_local.doFit(Stage2MaxCalls, Stage2MaxIters, Stage2Tolerance, varNames,
//                     seed, stepSize, localPar, localErr, chi2_penalized_local, fixedUt42);

//       double chi2_local = tf_local.CalculateChi2(localPar);
...
//         for (int j = 0; j < 4; ++j) {
//           thread_bestPar[j] = localPar[j];
//           thread_bestErr[j] = localErr[j];
//         }
//       }
//     }
...
//   // Final update after parallel block
//   bestChi2 = bestChi2_local;
//   for (int j = 0; j < 4; ++j) {
//     par_bf[j] = bestPar_local[j];
//     par_err[j] = bestErr_local[j];
...
int main(int argc, char const *argv[])
{
  ...
  //                     Form("%s/FCclosure/%s/plots/bestfit_%d", data_path, testName.c_str(), u));
  //   }
  ...
  //   if(trueSeeding){
  //     auto step3 = GetStage3StepSizes();
  //     tf.doFit(Stage3MaxCalls, Stage3MaxIters, Stage3Tolerance, varNames, tgtPar, step3.data(), bestfit.data(), bestfitError.data(), chi2_penalized, fixedUt42);
  //     bfchi2 = tf.CalculateChi2(bestfit.data());
  //   }

  //   else{  
  //     bfchi2 = RunThreeStageFit(tf, bestfit, bestfitError, nPoints_dm2, fixedUt42, print);
  //     // if(tgtchi2 - bfchi2 < 0 || tgtchi2 - bfchi2 > 40) 
  //     //   plotBestfit = true;
  //     // else 
  //     //   plotBestfit = false;
  //     tf.CalculateChi2( bestfit.data(), plotBestfit, Form("%s/FCclosure/%s/plots/bestfit_%d",data_path,testName.c_str(),u) ); 
  //   }
