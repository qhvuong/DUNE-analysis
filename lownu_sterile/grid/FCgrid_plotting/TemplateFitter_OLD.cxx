    ...
    bool plotBestfit) const
{
  ...
  assert(nbins_nue == 8  && "Expecting 8 bins for nu+e segment");
  ...
  const double edgesC_arr[9] = { 0.0, 0.3, 0.6, 0.92, 1.3, 1.75, 2.45, 3.9, 16.0 };
  std::vector<double> EC(edgesC_arr, edgesC_arr + 9);
  ...
  THStack* h_stack = new THStack(
      "h_stack",
      Form("Oscillation Hypothesis: U_{e4}^{2} = %.2f, U_{#mu4}^{2} = %.2f, U_{#tau4}^{2} = %.1f, #Deltam^{2} = %.1f eV^{2} (#nu < %.1f GeV)",
  ...
  // --- Split canvas: top = stack, bottom = ratio ---
  c->cd();
  c->Clear();
  TPad* pTop = new TPad("pTop","", 0.00, 0.30, 1.00, 1.00);
  ...
  gPad->Update();  // make sure the title TPaveText exists

  if (auto* pt = (TPaveText*)gPad->GetPrimitive("title")) {
    pt->SetTextSize(0.040);   // try 0.04–0.07; it's in NDC
    // pt->SetTextFont(62);
    pt->SetBorderSize(0);
    pt->SetFillStyle(0);
    // optional: reposition/resize the title box
    pt->SetX1NDC(0.12); pt->SetX2NDC(0.88);
    pt->SetY1NDC(0.93); pt->SetY2NDC(0.99);
    gPad->Modified(); gPad->Update();
  }

  // Set Y-title AFTER Draw() (stack frame exists now)
  if (TH1* frame = h_stack->GetHistogram()) {
    frame->GetYaxis()->SetTitle("entries / GeV / yr.POT");
    frame->GetXaxis()->SetTitle("Reconstructed lepton energy [GeV]");
    frame->GetXaxis()->SetTitleSize(0.045);
    frame->GetXaxis()->SetTitleOffset(1.2);  // tweak if needed
  }

  // Make room for labels below the frame
  gPad->SetBottomMargin(0.20);
  c->Update();

  // segment geometry (use your existing EA_trunc/EB/EC)
  auto span = [](const std::vector<double>& E){ return E.back() - E.front(); };
  ...
  // y at the exact bottom of the frame (user coords, log-safe)
  const double y0 = std::pow(10.0, gPad->GetUymin());  // <-- was gPad->GetUymin()

  // Divisions and seam tweak
  const int nDivA = 203;   // 0,1,2,3,4,5
  const int nDivBC = 505;  // 0,4,8,12,16
  const double eps0 = 1e-3;

  // FIRST segment (with title)
  auto* axA = new TGaxis(offA, y0, offA + spanA, y0, 0.0, spanA, nDivA, "+");
  axA->SetLabelSize(0.035);
  axA->SetLabelOffset(0.01);
  axA->SetLabelColor(kBlack);
  axA->SetLineColor(kBlack);
  axA->SetTickSize(0.02);
  axA->SetMaxDigits(3);
  axA->Draw();

  // SECOND segment (hide 0 with lower epsilon, keep 16)
  auto* axB = new TGaxis(offB, y0, offB + spanB, y0, 0.0, Emax, nDivBC, "+");
  axB->SetLabelSize(0.035);
  axB->SetLabelOffset(0.01);
  axB->SetLabelColor(kBlack);
  axB->SetLineColor(kBlack);
  axB->SetTickSize(0.02);
  axB->SetMaxDigits(3);
  axB->Draw();

  // THIRD segment (hide 0 with lower epsilon, keep 16)
  auto* axC = new TGaxis(offC, y0, offC + spanC, y0, eps0, Emax, nDivBC, "+");
  axC->SetLabelSize(0.035);
  axC->SetLabelOffset(0.01);
  axC->SetLabelColor(kBlack);
  axC->SetLineColor(kBlack);
  axC->SetTickSize(0.02);
  axC->SetMaxDigits(3);
  axC->Draw();

  // Keep the frame x-title (below the labels)
  if (auto* frame = h_stack->GetHistogram()) {
    frame->GetXaxis()->SetTitle("Reconstructed lepton energy [GeV]");
    frame->GetXaxis()->SetTitleSize(0.045);
    frame->GetXaxis()->SetTitleOffset(1.2);
  }
  // Hide only native x labels/ticks
  h_stack->GetXaxis()->SetLabelSize(0);
  h_stack->GetXaxis()->SetTickLength(0);

  // Place the custom axes a bit above the visible minimum (log-safe)
  const double yMinPlot = h_stack->GetMinimum();         // e.g., 10
  const double yAxis    = yMinPlot * 1.15;               // 15% above bottom

  // Separators at true ends of A and B
  const double yBottom = std::pow(10.0, gPad->GetUymin());
  const double yTop    = std::pow(10.0, gPad->GetUymax());
  // const double yStart  = std::max(yAxis * 1.01, yBottom * 1.001);

  auto* l1 = new TLine(offA + spanA, yBottom, offA + spanA, yTop); // end of A (≈5 GeV)
  auto* l2 = new TLine(offB + spanB, yBottom, offB + spanB, yTop); // end of B (16 GeV)

  for (TLine* L : {l1, l2}) { L->SetLineColor(kBlack); L->SetLineWidth(2); L->SetLineStyle(2); L->Draw(); }

  // Segment labels at centers
  const double yMid = std::sqrt(yBottom * yTop);
  TLatex latex; latex.SetNDC(false); latex.SetTextAlign(22); latex.SetTextSize(0.045);
  latex.DrawLatex(offA + 0.5*spanA, yMid, "#nu_{#mu}-CC");
  latex.DrawLatex(offB + 0.5*spanB, yMid, "#nu_{e}-CC");
  latex.DrawLatex(offC + 0.5*spanC, yMid, "#nu+e");
  // latex.DrawLatex(offB + 0.8*spanB, std::sqrt(yMid * yTop)*4.5, "#nu < 0.3 GeV");

  // Legend
  TLegend* leg = MakeLegend(0.60, 0.65, 0.82, 0.88);
  leg->AddEntry(hCCm_sig, "CC #nu_{#mu} inclusive");
  leg->AddEntry(hCCe_sig, "CC #nu_{e} inclusive");
  leg->AddEntry(hCCm_bkg_NC, "NC background");
  leg->AddEntry(hnue_sig, "#nu + e elastic");
  // if (hOsc_em && !is_empty(hOsc_em)) leg->AddEntry(hOsc_em, "Oscillated #nu_{e} #rightarrow #nu_{#mu}");
  // if (hOsc_me && !is_empty(hOsc_me)) leg->AddEntry(hOsc_me, "Oscillated #nu_{#mu} #rightarrow #nu_{e}");
  ...
  if (hRatio) {
    ...
    // Y range centered around 1
    double rMin = 0.5, rMax = 1.5;  // tweak if you like
    r->SetMinimum(rMin);
    r->SetMaximum(rMax);
    ...
    // Draw native axis (we’ll overwrite the x labels with composite axes below)
    r->Draw("hist");
    ...
    // Hide native x labels/ticks; re-draw composite 3-piece axis at y = bottom
    r->GetXaxis()->SetLabelSize(0);
    r->GetXaxis()->SetTickLength(0);

    const double y0_bot = gPad->GetUymin(); // linear y in ratio pad
    const int nDivA = 203, nDivBC = 505;
    const double Emax = 16.0, eps0 = 1e-3;

    // A
    auto* axA2 = new TGaxis(offA, y0_bot, offA + spanA, y0_bot, 0.0, spanA, nDivA, "+");
    axA2->SetLabelSize(0.08); axA2->SetLabelOffset(0.01); axA2->SetTickSize(0.04); axA2->Draw();
    // B
    auto* axB2 = new TGaxis(offB, y0_bot, offB + spanB, y0_bot, 0.0, Emax, nDivBC, "+");
    axB2->SetLabelSize(0.08); axB2->SetLabelOffset(0.01); axB2->SetTickSize(0.04); axB2->Draw();
    // C
    auto* axC2 = new TGaxis(offC, y0_bot, offC + spanC, y0_bot, eps0, Emax, nDivBC, "+");
    axC2->SetLabelSize(0.08); axC2->SetLabelOffset(0.01); axC2->SetTickSize(0.04); axC2->Draw();

  } else {
    // If nullptr, leave pad empty but labeled
    TLatex t; t.SetNDC(); t.SetTextSize(0.10); t.SetTextAlign(22);
    t.DrawLatex(0.5, 0.5, "No ratio available");
  }
...
TMatrixD TemplateFitter::BuildPredictionVector(const double *par, bool plotBestfit, const char* plotName) const {
  ...
  if ( plotBestfit ) {
    ...
    c->SaveAs(Form("%s_dm2.png",plotName));
    c->Close();
  }
