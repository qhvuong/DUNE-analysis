...
TemplateFitter::TemplateFitter(TemplateGroup_CC (&ccTemplates)[nbins_Ev],
                               TemplateGroup_nue (&nueTemplates)[nbins_Ev],
                               const std::vector<int>& dropBins,
                               bool noLimit,
                               bool PointLikeSource)
  : dropBins_(dropBins),
    noLimitCaps(noLimit),
    fixedL(PointLikeSource),
    par_min{ kParMinDefault[0], kParMinDefault[1], kParMinDefault[2], kParMinDefault[3] },
    par_max( GetParMax(noLimit) )  // uses the inline helper from the header
{
  ...
  std::cout 
            ...
            << "TemplateFitter bounds (noLimit=" << (noLimitCaps ? "true" : "false") << ")\n"
            << "  par_min = [" << par_min[0] << ", " << par_min[1]
            << ", " << par_min[2] << ", " << par_min[3] << "]\n"
            << "  par_max = [" << par_max[0] << ", " << par_max[1]
            << ", " << par_max[2] << ", " << par_max[3] << "]\n";
}
