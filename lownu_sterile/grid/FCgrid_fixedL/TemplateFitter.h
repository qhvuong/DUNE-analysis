...
static const double kParMinDefault[4] = {0., 0., 0., 0.};
// Original "tight" caps and the "no-limit" (1.0) caps for first three
static const double kParMaxTight[4] = {0.394, 0.489, 0.718, 1000.};
static const double kParMaxLoose[4] = {1.0,   1.0,   1.0,   1000.};
// Runtime selector for par_max
inline std::array<double,4> GetParMax(bool noLimit) {
  return noLimit ? std::array<double,4>{kParMaxLoose[0], kParMaxLoose[1], kParMaxLoose[2], kParMaxLoose[3]}
                 : std::array<double,4>{kParMaxTight[0], kParMaxTight[1], kParMaxTight[2], kParMaxTight[3]};
}


//Neutrino bins
static const int nbins_Ev = 430;
static const int nbins_Ev_LE = 12;
...
struct LE_FitParameters {
  TH1D* LEdep_m[nbins_Ev_LE];
  TH1D* LEdep_e[nbins_Ev_LE];
  double LEfit_m[nbins_Ev_LE][7];
  double LEfit_e[nbins_Ev_LE][7];
};
...
class TemplateFitter {
  public:
    TemplateFitter(TemplateGroup_CC (&ccTemplates)[nbins_Ev], TemplateGroup_nue (&nueTemplates)[nbins_Ev], const std::vector<int>& dropBins = {}, bool noLimit = false, bool PointLikeSource = false);
    ~TemplateFitter() {}
  ...
  private:
    double getPme(double energy, double ue42, double um42, double ut42, double dm2, double L = 0.5 /* km */) const;
    double getPmt(double energy, double ue42, double um42, double ut42, double dm2, double L = 0.5 /* km */) const;
    double getPet(double energy, double ue42, double um42, double ut42, double dm2, double L = 0.5 /* km */) const;
    double getPee(double energy, double ue42, double um42, double ut42, double dm2, double L = 0.5 /* km */) const;
    double getPmm(double energy, double ue42, double um42, double ut42, double dm2, double L = 0.5 /* km */) const;
    ...
    bool noLimitCaps;
    bool fixedL;
    std::array<double,4> par_min;
    std::array<double,4> par_max;
};


namespace FitUtils {

// inline std::vector<std::pair<double, double>> GetDM2SeedsAndSteps() {
//     return {
//         {0.1, 1.0}, {0.4, 1.0}, {0.7, 1.0}, {1.0, 1.0}, 
//         {2.0, 1.0}, {4.0, 1.0}, {7.0, 1.0}, {10.0, 2.0}, 
//         {15.0, 2.0}, {25.0, 5.0}, {60.0, 5.0}, {150.0, 10.0}
//     };
// }
...
GetStage1SeedSets(bool noLimit) {
    std::array<double,4> pmin{ kParMinDefault[0], kParMinDefault[1], kParMinDefault[2], kParMinDefault[3] };
    auto pmax = GetParMax(noLimit);
    return {
        {pmin[0], pmin[1], pmin[2], pmin[3]},
        {pmax[0], pmax[1], pmax[2], 100.0},
        {pmin[0], pmin[1], pmin[2], 100.0},
        {pmax[0], pmax[1], pmin[2], pmin[3]}
    };
}
