...
TemplateFitter::TemplateFitter(TemplateGroup_CC (&ccTemplates)[nbins_Ev], TemplateGroup_nue (&nueTemplates)[nbins_Ev], const std::vector<int>& dropBins, bool PointLikeSource) 
: dropBins_(dropBins),
  fixedL(PointLikeSource)
{
...
double TemplateFitter::getPme( double energy, double ue42, double um42, double ut42, double dm2, double L) const
{
  //double L = 0.5;
  double L2 = 0.574, L1 = 0.35;
  double del = 1.27*L*dm2/energy;
  double s2me2 = 4 * ue42 * um42;
  double prob = s2me2  * pow(sin(del),2);
  return prob;
}
double TemplateFitter::getPmt( double energy, double ue42, double um42, double ut42, double dm2, double L) const
{
  double del = 1.27*L*dm2/energy;
  double s2mt2 = 4 * um42 * ut42;
  double prob = s2mt2 * pow(sin(del),2);
  return prob;
}
double TemplateFitter::getPet( double energy, double ue42, double um42, double ut42, double dm2, double L) const
{
  double del = 1.27*L*dm2/energy;
  double s2et2 = 4 * ue42 * ut42;
  double prob = s2et2 * pow(sin(del),2);
  return prob;
}
double TemplateFitter::getPee( double energy, double ue42, double um42, double ut42, double dm2, double L) const
{
  //double L = 0.5;
  double del = 1.27*L*dm2/energy;
...
double TemplateFitter::getPmm( double energy, double ue42, double um42, double ut42, double dm2, double L) const
{
  //double L = 0.5;
  double del = 1.27*L*dm2/energy;
    ...
    bool plotBestfit) const
{
  ...
  // components (totals per channel across the flattened bins)
  ...
  TH1D* h_nc = new TH1D("h_ne","", nbins, 0, nbins);
  h_mm->SetDirectory(nullptr);
  h_me->SetDirectory(nullptr);
  h_ee->SetDirectory(nullptr);
  h_em->SetDirectory(nullptr);
  h_nc->SetDirectory(nullptr);

  // ---- CC νμ region: bins [0 .. 54)
  // signal only (you don’t have explicit CCνμ backgrounds here)
  for (int i = 0; i < nbinsCC; ++i) {
    h_mm->SetBinContent(i+1, CCm_sig_mm->GetBinContent(i+1)); // νμ→νμ
    h_em->SetBinContent(i+1, CCm_sig_em->GetBinContent(i+1)); // νe→νμ
    // h_ee, h_me remain 0 in this region
    h_nc->SetBinContent(i+1, CCm_bkg_nc->GetBinContent(i+1)); // νμ→νμ
  }

  // ---- CC νe region: bins [54 .. 108)
  // add SIGNAL (ee, me) + BACKGROUND (mm, em, ee, me) into their channels
  for (int i = nbinsCC; i < 2*nbinsCC; ++i) {
    const int idx = i - nbinsCC;
    ...
    h_ee->SetBinContent(i+1, sig_ee + bkg_ee);
    h_me->SetBinContent(i+1, sig_me + bkg_me);
    h_mm->SetBinContent(i+1, bkg_mm); // background contributes to mm channel in CC νe region
    h_em->SetBinContent(i+1, bkg_em); // background contributes to em channel in CC νe region
    h_nc->SetBinContent(i+1, bkg_nc); // background contributes to em channel in CC νe region
  }

  // ---- ν–e region: bins [108 .. 116)
  // add SIGNAL (mm, me, ee, em) + BACKGROUND (ee, em from CC νe leakage)
  for (int i = 2*nbinsCC; i < 2*nbinsCC + nbins_nue; ++i) {
    const int idx = i - 2*nbinsCC;
    ...
    h_mm->SetBinContent(i+1, s_mm);
    h_me->SetBinContent(i+1, s_me);
    h_ee->SetBinContent(i+1, s_ee + b_ee);
    h_em->SetBinContent(i+1, s_em + b_em);
  }
  ...
  auto color_nc = dunestyle::colors::NextColor(dunestyle::colors::Cycle::OkabeIto, 3);
  ...
  h_nc->SetFillColor(color_nc);  // NC component

  // for(int i=0; i<nbins; i++){
  //   std::cout << h_nc->GetBinContent(i+1) << "\n";
  // }
  ...
  leg->AddEntry(h_nc,"NC component");
  
  if(plotBestfit){
    tgt->SetLineColor(dunestyle::colors::NextColor(dunestyle::colors::Cycle::OkabeIto, 4));
    FCtgt->SetLineColor(dunestyle::colors::NextColor(dunestyle::colors::Cycle::OkabeIto, 5));
...
TMatrixD TemplateFitter::BuildPredictionVector(const double *par, bool plotBestfit, const char* plotName) const {
  ...
  for (int i = 10; i < nbins_Ev; ++i) {
    ...
    if(!fixedL){
      ...
      if(nbins_Ev_LE == 29) {
        ...
        // THIS IS UPDATED MAPPING FOR 12 ENERGY BANDS
      ...
      for (int j = 0; j <= 1000; ++j) {
        double e = binEdges[i] + j * (binEdges[i+1] - binEdges[i]) / 1000.0;
        me += getPme(e, par[0], par[1], par[2], par[3]);
        mt += getPmt(e, par[0], par[1], par[2], par[3]);
        em += getPme(e, par[0], par[1], par[2], par[3]);
        et += getPet(e, par[0], par[1], par[2], par[3]);
        ee += getPee(e, par[0], par[1], par[2], par[3]);
        mm += getPmm(e, par[0], par[1], par[2], par[3]);
      }      
    }
  ...
  // 3. Fill TMatrixD
  // For printing out bin number, bin width, and bin content:
  double CCEdges[nbinsCC+1];
  CCEdges[0] = 0.;
  for (int i = 0; i < nbinsCC; i++) {
    if (i < 1)       CCEdges[i + 1] = CCEdges[i] + 0.45;
    else if (i < 35) CCEdges[i + 1] = CCEdges[i] + 0.1;
    else if (i < 36) CCEdges[i + 1] = CCEdges[i] + 0.15;
    else if (i < 41) CCEdges[i + 1] = CCEdges[i] + 0.2;
    else if (i < 46) CCEdges[i + 1] = CCEdges[i] + 0.4;
    else if (i < 51) CCEdges[i + 1] = CCEdges[i] + 0.8;
    else if (i < 53) CCEdges[i + 1] = CCEdges[i] + 1.5;
    else             CCEdges[i + 1] = CCEdges[i] + 2.0;
  }
  double NuEEdges[nbins_nue+1] = {0.3, 0.6, 0.92, 1.3, 1.75, 2.45, 3.9, 16.0};

  TMatrixD prediction(nbins, 1);
  for (int bx = 0; bx < nbins; ++bx) {
    if (bx < nbinsCC){
      prediction[bx][0] = CCm_tp->GetBinContent(bx + 1);
      // std::cout << bx+1 << "\t & \t" << std::fixed << std::setprecision(2) << "[" << CCEdges[bx] << ", " << CCEdges[bx+1] << "]" << "\t & \t" << std::fixed << std::setprecision(1) << prediction[bx][0] << "\t\\\\\n";
    }
    else if (bx < 2 * nbinsCC){
      prediction[bx][0] = CCe_tp->GetBinContent(bx - nbinsCC + 1);
      // std::cout << bx+1 << "\t & \t" << std::fixed << std::setprecision(2) << "[" << CCEdges[bx-nbinsCC] << ", " << CCEdges[bx-nbinsCC+1] << "]" << "\t & \t" << std::fixed << std::setprecision(1) << prediction[bx][0] << "\t\\\\\n";
    }
    else{
      prediction[bx][0] = nue_tp->GetBinContent(bx - 2 * nbinsCC + 1);
      // std::cout << bx+1 << "\t & \t" << std::fixed << std::setprecision(2) << "[" << NuEEdges[bx-2*nbinsCC] << ", " << NuEEdges[bx-2*nbinsCC+1] << "]" << "\t & \t" << std::fixed << std::setprecision(1) << prediction[bx][0] << "\t\\\\\n";
    }
  ...
  if ( plotBestfit ) {
    ...
    c->SaveAs(Form("%s_wBKG.png",plotName));
    c->Close();
  }
  ...
  // for(int i=0; i<nbins; i++){
  //   // if(i<nbinsCC) std::cout << i+1 << "\t & \t" << CCEdges[i+1]-CCEdges[1] << "\t & \t" << prediction[i][0] << "\n";
  //   std::cout << i+1 << "\t & \t" << prediction[i][0] << "\n";
  // }
...
double TemplateFitter::getChi2(const double* par) {
  ...
  double epsilon = 1E-8;
  if (par[3] < 0.1) penalty = (0.1 / (par[3] + epsilon)) * (0.1 / (par[3] + epsilon));
  else if (par[3] > 100.) penalty = (par[3] - 100.) * (par[3] - 100.);
