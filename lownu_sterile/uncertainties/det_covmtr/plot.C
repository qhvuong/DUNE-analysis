...
const char data_path[] = "/exp/dune/app/users/qvuong/data/lownu/uncertainties/det_covmtr";
...
void plot()
{
  TFile *f = new TFile(Form("%s/det_covmtr.root",data_path), "READ");
  ...
  for(int j=0; j<N_nucut; j++){
    ...
    hfrcv[j]->SetMinimum(1E-3);
    hfrcv[j]->SetMaximum(0.5);
