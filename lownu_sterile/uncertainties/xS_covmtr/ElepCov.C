...
void covariance_pn(TH1D* hnom, TH1D* hp, TH1D *hn, TH2D* cov, TCanvas* c, const char *name){
  ...
  //makes lines to divide matrix to show the three different samples
    ...
    l4->Draw("same");
    c->SaveAs(Form("xScov/%s.png",name));
  */
}
...
void covariance(TH1D* hnom, TH1D* h, TH2D* cov, TH2D* frCov, TCanvas* c, const char *name){
  ...
  //makes lines to divide matrix to show the three different samples
    ...
    c->Clear();
    frCov->SetMinimum(1E-3);
    frCov->SetMaximum(1.);
    frCov->SetStats(0);
    frCov->Draw("colz");
    c->SaveAs(Form("xScov/fractional_%s.png",name));
...
void ElepCov()
{
  ...
  for(int j=0; j<N_nucut; j++){
    ...
    for(int iw=0; iw<N_wgt+1; iw++){
      ...
      if(iw<N_wgt){
        ...
        covariance_pn(ho, hp, hn, hcv[j][iw], c, Form("%s_%d",name[iw],j));
      }
        ...
        covariance_pn(ho, hp, hn, hcv[j][iw], c, Form("nue_%d",j));
      }
    }
  }
  ...
  TFile *out = new TFile(Form("%s/uncertainties/xS_covmtr/xS_covmtr.root",data_path), "RECREATE");
  for(int j=0; j<N_nucut; j++){
  for(int iw=0; iw<N_wgt+1; iw++){
    hcv_n[j][iw]->Write();
    hcv[j][iw]->Write();
