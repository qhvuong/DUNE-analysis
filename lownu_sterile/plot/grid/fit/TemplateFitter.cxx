...
double s0,s1,s2;
double L_tg, L_tp;
void TemplateFitter::setPara( char var[20], double oscpar[3], int nuCut, double seed[3], double fitPara_m[29][7], double fitPara_e[29][7] )
{
  name  = var;
  cutNu = nuCut;
  for(int i = 0; i < 3; i++){
...
double TemplateFitter::noChi2 ( double *par )
{
  // Create a histogram "temp" from the templates
  // Start with the intrinsic nu_e CC template, which doesn't change with oscillations
  TH1D * CC_tp_e = (TH1D*) CC_m_templates[0]->Clone();
  ...
  for( int i = 10; i < nbins_Ev; ++i ) {
    ...
    for(int j = 0; j<1001; j++){
      double e = m_energy_bins[i] + j*(m_energy_bins[i+1] - m_energy_bins[i])/1000.;
      mue = mue + getPmue(e, noPar[0], noPar[1], noPar[2], L); // par[0] = Uee2, par[1] = Umm2, par[2] = dm2
      emu = emu + getPmue(e, noPar[0], noPar[1], noPar[2], L); // par[0] = Uee2, par[1] = Umm2, par[2] = dm2
      ee  = ee  + getPee(e, noPar[0], noPar[1], noPar[2], L);
      mm  = mm  + getPmm(e, noPar[0], noPar[1], noPar[2], L);
    }
    double Pmue = mue/1001.0;
    double Pemu = emu/1001.0;
    double Pee  = ee/1001.0;
    double Pmm  = mm/1001.0;
    //std::cout << Pmue << "\t" << Pee << "\t" << Pmm << "\n";
  ...
  //std::cout << "bestfit values: " << noPar[0] << "\t" << noPar[1] << "\t" << noPar[2] << "\t" << chi2 << "\n";
