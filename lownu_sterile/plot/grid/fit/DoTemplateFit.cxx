...
*/
  ...
  int point;
  ifstream para("../NuCut_dm1.txt");
  while (!para.eof()) {
    para >> point >> oscpar[0] >> oscpar[1] >> oscpar[2];
    cout << point << "\n";
    ...
    tf.setPara( var, oscpar, nuCut, seed, fitPara_m, fitPara_e );
    ...
    ofstream myfile;
    //myfile.open(Form("output_%d.txt",point));
    myfile.open(Form("output_%d.txt",point));
    myfile << oscpar[0] << "\t" << oscpar[1] << "\t" << oscpar[2] << "\t" << sen << "\n";
    myfile.close();
