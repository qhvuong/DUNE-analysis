...
double TemplateFitter::getChi2( double * par )
{
  ...
  TH2D *hCov = new TH2D("hCov","",nbins,0,nbins,nbins,0,nbins);
  TH2D *hInv = new TH2D("hInv","",nbins,0,nbins,nbins,0,nbins);
  for( int bx = 0; bx < nbins; bx++ ) {
    for( int by = 0; by < nbins; by++ ) {
      hCov->SetBinContent(bx+1, by+1, covmtr[bx][by]);
      hInv->SetBinContent(bx+1, by+1, inv[bx][by]);
  }
  }
