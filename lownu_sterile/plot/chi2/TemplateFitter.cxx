...
void TemplateFitter::setPara( char var[20], int iUt, int iUm, double par[4], double fitPara_m[29][7], double fitPara_e[29][7] )
{
  name  = var;
...
void TemplateFitter::Draw()
{
  ...
  for(int i=1; i<=N; i++) {
    ...
    for(int j=1; j<=N; j++) {
      ...
      if( j==N ) std::cout << par[0] << "\t" << par[1] << "\t" << par[2] << "\t" << par[3] << "\t" << chi2 << "\t" << diff << "\t" << i*j*100./(N*N) << "%" << "\n";
...
/*  
  ...
  for(int i=1; i<=N; i++) {
    ...
    for(int j=1; j<=N; j++) {
      ...
      chi2  = getChi2(par);
      diff  = sqrt(std::fabs(chi2 - chi2t));
      if( j==N ) std::cout << par[0] << "\t" << par[1] << "\t" << par[2] << "\t" << par[3] << "\t" << chi2 << "\t" << diff << "\t" << i*j*100./(3*N*N) << "%" << "\n";
...
*/
  ...
  TCanvas *cchi2 = new TCanvas("cchi2","",600,500);
  //cchi2->Divide(3,1);
  //cchi2->cd(1);
  gPad->SetGrid();
  gPad->SetLogx();
...
/*
  ...
  g23->Draw("same C*");
*/
  cchi2->SaveAs(Form("%s/chi2Surface_%d%d.png",data_path,it,im)); 
...
*/
  TFile *out = new TFile(Form("%s/chi2_%d%d.root",data_path,it,im),"RECREATE");
