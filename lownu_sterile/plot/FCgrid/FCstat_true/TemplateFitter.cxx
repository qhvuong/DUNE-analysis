...
void TemplateFitter::getTarget( double *par )
{
  ...
  for(int i=0; i<nbins; i++){
    if(i<nbins_CC){
      ...
      //cout << i << "\t" << bct << "\t" << CCm_tgt->GetBinContent(b+1) << "\n";
    }
    ...
    if(i>=nbins_CC && i<2*nbins_CC){
      ...
      //cout << i << "\t" << bct << "\t" << CCe_tgt->GetBinContent(b+1) << "\n";
    }
    ...
    if(i>=2*nbins_CC){
      ...
      //cout << i << "\t" << bct << "\t" << nue_tgt->GetBinContent(b+1) << "\n";
    }
  }
  ...
  for(int by=0; by<nbins; by++)  {
    if(bx < nbins_CC) {
      if(by < nbins_CC)                     flmx[bx][by] = flmx[bx][by] * CCm_tgt_thr->GetBinContent(bx+1) * CCm_tgt_thr->GetBinContent(by+1);
      if(by >= nbins_CC && by < 2*nbins_CC) flmx[bx][by] = flmx[bx][by] * CCm_tgt_thr->GetBinContent(bx+1) * CCe_tgt_thr->GetBinContent(by-nbins_CC+1);
      if(by >= 2*nbins_CC)                  flmx[bx][by] = flmx[bx][by] * CCm_tgt_thr->GetBinContent(bx+1) * nue_tgt_thr->GetBinContent(by-2*nbins_CC+1);
    }
    if(bx >= nbins_CC && bx < 2*nbins_CC) {
      if(by < nbins_CC)                     flmx[bx][by] = flmx[bx][by] * CCe_tgt_thr->GetBinContent(bx-nbins_CC+1) * CCm_tgt_thr->GetBinContent(by+1);
      if(by >= nbins_CC && by < 2*nbins_CC) flmx[bx][by] = flmx[bx][by] * CCe_tgt_thr->GetBinContent(bx-nbins_CC+1) * CCe_tgt_thr->GetBinContent(by-nbins_CC+1);
      if(by >= 2*nbins_CC)                  flmx[bx][by] = flmx[bx][by] * CCe_tgt_thr->GetBinContent(bx-nbins_CC+1) * nue_tgt_thr->GetBinContent(by-2*nbins_CC+1);
    }
    if(bx >= 2*nbins_CC) {
      if(by < nbins_CC)                     flmx[bx][by] = flmx[bx][by] * nue_tgt_thr->GetBinContent(bx-2*nbins_CC+1) * CCm_tgt_thr->GetBinContent(by+1);
      if(by >= nbins_CC && by < 2*nbins_CC) flmx[bx][by] = flmx[bx][by] * nue_tgt_thr->GetBinContent(bx-2*nbins_CC+1) * CCe_tgt_thr->GetBinContent(by-nbins_CC+1);
      if(by >= 2*nbins_CC)                  flmx[bx][by] = flmx[bx][by] * nue_tgt_thr->GetBinContent(bx-2*nbins_CC+1) * nue_tgt_thr->GetBinContent(by-2*nbins_CC+1);
    }
  }}
