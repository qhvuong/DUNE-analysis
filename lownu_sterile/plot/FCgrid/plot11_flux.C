void plot11_flux()
{
  ...
  for(int i=0; i<N; i++) {
    ifstream f(Form("/pnfs/dune/scratch/users/qvuong/output/flux_new/s%s/output_%d.txt",seed,i));
  ...
  chi2Cor->Draw("colz");
  c->SaveAs(Form("new/chi2Cor_%s_flux.png",seed));
  ...
  c1->SaveAs(Form("new/dChi2Vsdm2_%s_flux.png",seed));
  ...
  c2->SaveAs(Form("new/dChi2VsUmm2_%s_flux.png",seed));
  ...
  c3->SaveAs(Form("new/dChi2VsUee2_%s_flux.png",seed));
  ...
  c4->SaveAs(Form("new/dm2VsUmm2_%s_flux.png",seed));
  ...
  c5->SaveAs(Form("new/dm2VsUee2_%s_flux.png",seed));
  ...
  c6->SaveAs(Form("new/Uee2VsUmm2_%s_flux.png",seed));

  TFile *out = new TFile(Form("new/seed%s_flux.root",seed),"RECREATE");
  chi2Cor->Write();
  dChi2Vsdm2->Write();
  dChi2VsUmm2_s->Write();
  dChi2VsUmm2_m->Write();
