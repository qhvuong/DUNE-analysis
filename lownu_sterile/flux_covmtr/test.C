...
*/
  ...
  TMatrixD ECov( 2*nbins_E, 2*nbins_E );
  ...
  for(int i = 0; i < 2*nbins_E; i++) {
    for(int j = 0; j < 2*nbins_E; j++) {
      ECov[i][j] = ECovars[i][j];
    }
  }


  TH2D *hcv = new TH2D("hcv","",300,0,300,300,0,300);
  TH2D *hcv_test = new TH2D("hcv","",200,0,200,200,0,200);
  ...
  for(int i=0; i<200; i++) {
    for(int j=0; j<200; j++) {
      hcv_test->SetBinContent(i+1, j+1, ECov[i][j]);
    }
  }
  ...
  TCanvas *ccv_test = new TCanvas("ccv_test","",900,800);
  hcv_test->Draw("colz");
  ccv_test->SaveAs(Form("%s_Cov_test%d%d%d_%d.png",name,para,cutNu,cutEv,N));
